...
class CameraGridWidget(QWidget):
    ...
    def refresh_grid(self):
        ...
        for camera_id, camera_info in cameras.items():
            # Make sure to create UI components here in the main thread
    ...
    def refresh_single_view(self):
        ...
        # Get the camera view
        view = self.camera_views.get(self.selected_camera_id)
        if not view:
            # Create the view if it doesn't exist
            camera = self.camera_manager.get_camera(self.selected_camera_id)
            if camera:
                view = CameraViewWidget(camera, None)
                view.frame_clicked.connect(lambda x, y: self.frame_clicked.emit(self.selected_camera_id, x, y))
                self.camera_views[self.selected_camera_id] = view
